...
do_configure_append() {
	...
	echo "/include/ \"${DTB_PL_DELETE}.dtsi\"" >> "${DT_FILES_PATH}/pl.dtsi"
	[ ${MACHINE} == "plnx-zynq7" ] && { \
		echo "#include \"zynq-stdout-path.dtsi\"" >> "${DT_FILES_PATH}/system-top.dts"; }
	# Remove system-conf since it touches in nodes that our dts already touch.
	sed -i '/system-conf.dtsi/d' "${WORKDIR}/system-user.dtsi"
