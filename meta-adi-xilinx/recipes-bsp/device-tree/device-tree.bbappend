...
# there's an issue in petalinux for microblaze designs using the uartlite core. we need to force it
# as suggested in here:
# https://forums.xilinx.com/t5/Embedded-Linux/AXI-Uartlite-issues-with-Petalinux-2020-1-and-Microblaze-systems/td-p/1126614
# the other option would be to directly patch petalinux after installing it but that could be cumbersome for some users...
DTS_CONSOLE_DEVICE_CONFIG ?= "axi_uart"
YAML_CONSOLE_DEVICE_CONFIG_microblaze_forcevariable = "${DTS_CONSOLE_DEVICE_CONFIG}"
...
do_configure_append() {
	...
	# As mentioned above, there's issues with uartlite and microblaze. Hence, the autogenerated properties are not ok.
	# Thus, we try to fix them during the configure task... Note that we are trying to just add the console options
	# without breaking other things like 'root='
	[[ ${ARCH} == "microblaze" ]] && { \
