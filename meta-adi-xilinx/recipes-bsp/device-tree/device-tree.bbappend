...
do_configure_append() {
	...
	# If we find a dts tag on the top devicetree, the pl.dtsi is included right after. This is specially important
	# for microblaze projects where the pl.dtsi should be the first thing included. This, because there are nodes that are
	# both defined in the pl.dtsi and in the base devicetree (eg: vc707.dtsi) and if we include the pl.dtsi after the base
	# dts we might permanently delete nodes that are needed afterwards. For projects were we do not have a dts tag on the
	# top devicetree (zynqMP), we include the pl.dtsi after the first #include it finds...
	if grep -qse "/dts-v1/" "${DT_FILES_PATH}/system-top.dts"; then
